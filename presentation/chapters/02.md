...
Programs (sketches) are written with the Arduino Programming Language (C++ subset)
...
no integrated capability to upload programs. All of the units in the previous
image are way harder to use than any Arduino.
...
When connected to a computer via USB it becomes powered on. Avoid touching the pins
with your hands as this might damage the device. Power it on when connected to
a breadboard.
...
The device appears on your computer as a serial port. This port is used to
upload (flash) programs to the device, as well as for bidirectional communication
in a console or in a programmatic environment.

???

Let's backtrack a bit to see some characteristics of these
...
The IDE requires that this file be named the same as its parent folder.
...
In this world this is a very important feature:
- Used to send programs to the arduino
- Can send and receive information from an arduino sketch (send commands or print debug information)
- Can be an interface between devices who wish to communicate with one another


Serial communication is always available via USB, but some arduinos have hardware serial pins.
