...
Talk about how this is going to be: the code is already written, we just have
to worry about setting up the circuits and understanding the code.

Talk about secrets.h
...
In Linux, you might have to give permission for your user to access the serial port. This is done via:

```
sudo usermod -a -G dialout $USER
```

Then log out and log back in. Alternatively you can run the Arduino IDE as root, but that's not a great idea.
...
labels on the board. Talk about pins.h.
...
void loop() {
  ...
  client.print("GET /Porto,Portugal?T HTTP/1.1\r\n");
...
## Exercise 3 &mdash; Use an IoT API to log values from a DHT11 sensor
...
## Exercise 4 &mdash; Use physical buttons to increment and decrement a counter
